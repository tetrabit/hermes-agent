...
def list_authenticated_providers(
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if not has_creds:
            try:
                from hermes_cli.auth import _load_auth_store, get_provider_auth_state, read_credential_pool

                store = _load_auth_store()
                providers_store = store.get("providers", {}) if isinstance(store, dict) else {}
                pool_store = store.get("credential_pool", {}) if isinstance(store, dict) else {}
                if (
                    get_provider_auth_state(pid) is not None
                    or get_provider_auth_state(hermes_slug) is not None
                    or read_credential_pool(pid)
                    or read_credential_pool(hermes_slug)
                    or pid in providers_store
